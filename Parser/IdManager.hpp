...
public:
	...
	size_t addSource(std::string label, std::string type){
		if(idMap[label]) return idMap[label]; //already defined -> don't add again
	...
	void propToSource(size_t node, size_t parent, size_t source) {
		...
		if(current["inputs"] == nullptr){
			if(current["type"]=="prop") {
				// here it's necessary to create a prop node connected to the source, to ensure uniqueness
		...
		for(auto inp : current["inputs"]) propToSource(inp, node, source);
	}
	...
	size_t addNode(json value, std::string type){
    	...
    	size_t id = get();
		diagram["nodes"][std::to_string(id)]= {{"text", value},{"type",type},{"id",id}};
		return id;    
    }

	void setNodeLR(size_t nodeId, size_t left, size_t right){
		// std::cerr << "set node lr " << nodeId << " "  << left <<  " "  << right << std::endl; 
	    auto node = diagram["nodes"].find(std::to_string(nodeId));
	    if(node != diagram["nodes"].end()){
            // simplifying the graph by assuming:
           // if 2 inputs are defined, the first one will be considered Left and the second Right
	...
	void prependNodeInput(size_t nodeId, size_t value){
		// std::cerr << "prepend node input " << nodeId << " "  << value << std::endl; 
	    auto node = diagram["nodes"].find(std::to_string(nodeId));
	    if(node != diagram["nodes"].end()){
	        if(!(*node)["inputs"].is_array()) (*node)["inputs"].push_back(value);
		    else (*node)["inputs"].insert((*node)["inputs"].begin(), value);
		}
	}  
