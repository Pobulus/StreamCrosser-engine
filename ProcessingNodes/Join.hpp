...
class Join : public IProcNode {
    ...
    void joinInputs() {
        if(!_callbacks.size()) throw EngineException("Dangling node: JOIN");
        vector<json> rows = {};
        const int leftLen = leftData["head"].size();
        const int rightLen = rightData["head"].size();
        vector<json> jointHead;
        jointHead.insert(jointHead.end(), leftData["head"].begin(), leftData["head"].end());
        jointHead.insert(jointHead.end(), rightData["head"].begin(), rightData["head"].end());
        json output;
...
public:
    Join(std::string type) {
        if(type == "Right Join") allRight = true;
        else if(type == "Left Join") allLeft= true;
    ...
    void dataInput(json data, int inputID) override {
        ...
        switch(inputID) {
            case 0:
                leftData = data;
                if(!rightData.empty()) joinInputs();
                break;
            case 1:
                rightData = data;
                if(!leftData.empty()) joinInputs();
                break;
            case 2:
                expression = data;
                break;
