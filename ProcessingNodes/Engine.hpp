...
#define ATTACH(X, Y, Z)                                                        \
  (X).attachOutput([Y, Z](json data) { (Y)->dataInput(data, Z); });
...
public:
  ...
  void configure(YAML::Node config) {
    ...
    server.Get("/str", [config](const httplib::Request & /*req*/,
    ...
    server.Post(
        "/query", [](const httplib::Request &req, httplib::Response &res) {
          string cmd = "./parser '" + req.body + "' > query.json";
          cout << "Parsing query" << req.body << endl;
          system(cmd.c_str());
          std::ifstream queryFS("query.json");
          std::string queryStr((std::istreambuf_iterator<char>(queryFS)),
    ...
    server.Post("/out",
                [this](const httplib::Request &req, httplib::Response &res) {
                  ...
                  if (this->outputs.count(name))
                    res.set_content(this->outputs[name]->getData().dump(),
                                    "application/json");
                });
    ...
    for (auto stream : config["streams"]) {
      ...
      server.Post("/str/" + name, [stream, this](const httplib::Request &req,
                                                 httplib::Response &res) {
        cout << "Stream data for: " << stream["name"] << ':' << req.body
             << std::endl;
        ...
        if (input) {
          ...
          res.set_content("no processing node connected to this stream",
                          "text/plain");
      });
      server.Get("/str/" + name, [stream](const httplib::Request &req,
                                          httplib::Response &res) {
        YAML::Emitter emitter;
        ...
        // {
        //     resp +=
        //     "\""+it->first.as<string>()+"\":"+"\""+it->second.as<string>()+"\",";
        // }
  ...
  void installQuery(json query) {
    ...
    // for(auto inp : inputTables) { cout << inp.first << endl;
    // inp.second->reset();}
    ...
    for (auto source : query["sources"]) {
      if (inputs.count(source["text"])) {
        processingNodes[source["id"]] =
            inputs[source["text"]]; // add to node map
        ...
        for (auto prop : query["props"].items()) {
          ...
          if (propSrc == srcId) {
            inputs[source["text"]]->selectProp(
                propName.substr(dotIndex + 1, string::npos));
          }
        }
      } else
        throw EngineException("No source with name " + (string)source["text"]);
    }
    ...
    for (auto node : query["nodes"]) {
      ...
      for (int i = 0; i < node["inputs"].size(); i++) {
        ...
        if (src != nullptr && dest != nullptr) {
          cout << "attaching " << node["inputs"][i] << " to " << id
               << " on input " << i << endl;
          ATTACH(*src, dest, i);
        }
      }
  ...
  void reload() {
    ...
    try {
      for (auto table : inputs) {
        ...
        table.second->dataInput("", 1); // this signals SourceTables to load
                                        // files but is ignored by SourceStreams
      }
    } catch (EngineException exc) {
      cout << exc.what() << endl;
    }
  ...
  void startServer() {
    ...
    int port = 8080;
    std::cout << "server listening on: http://" << addres << ":" << port
              << std::endl;
