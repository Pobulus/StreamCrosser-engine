  ...
  static function<json(json)> compileExpression(json expression, json head) {
    ...
    for (auto el : expression) {
      ...
      if (el.is_object()) {
        ...
        if (op == "==")
          opstack.push([left, right](json in) {
            auto l = left(in);
            auto r = right(in);
            return l == r;
          });
          ...
          opstack.push(
              [left, right](json in) { return left(in) != right(in); });
        else if (op == "<")
          opstack.push([left, right](json in) { return left(in) < right(in); });
        else if (op == ">")
          opstack.push([left, right](json in) { return left(in) > right(in); });
        else if (op == "<=")
          opstack.push(
          ...
          opstack.push([left, right](json in) {
            return (int)left(in) + (int)right(in);
          });
        else if (op == "-")
          opstack.push([left, right](json in) {
            return (int)left(in) - (int)right(in);
          ...
          opstack.push([left, right](json in) {
            return (double)left(in) / (double)right(in);
          });
        else if (op == "//")
          opstack.push([left, right](json in) {
            return (long)left(in) / (long)right(in);
          });
        else if (op == "/+")
          opstack.push([left, right](json in) {
            return (long)left(in) % (long)right(in);
          ...
          opstack.push([left, right](json in) {
            return pow((long)left(in), (long)right(in));
          });
        else if (op == "LIKE")
          throw EngineException("LIKE is not supported yet");
        else {
          throw EngineException("Unsupported operation [" + (string)op + "]");
        }
